...
from goldenfile.model import ExecutedTest, Reporter, TestSuiteExecutionResult
from goldenfile.reporters.base_reporter import BaseReporter


def save_report(to: Path) -> Reporter:
    def reporter(result: TestSuiteExecutionResult) -> None:
        res = []

        def add_as(category: str, seq: Sequence[ExecutedTest]) -> None:
            for t in seq:
                res.append(
                    {"name": t.test.name, "tags": t.test.tags, "result": category}
                )
            pass
